...
# unpickle the confusion matrix for the noise experiments
f_noise = open('./outputs/CM_noise_LV.pkl','rb')

CM = pickle.load(f_noise)
...
xvals = []
yvals = []
for noise in CM.keys():
    cm = np.array(CM[noise],dtype='float64')
    accuracy = (cm[0,0] + cm[1,1]) / np.sum(cm) * 100.
    x = noise
    y = accuracy
    xvals.append(x)
